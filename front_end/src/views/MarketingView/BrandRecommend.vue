<template>
  <div class="common-layout">
    <el-container>
      <el-header class="yanse">用来存放{{ tableData[0]}}和数据列表</el-header>
      <el-main>
        <el-table :data="tableData" border style="width: 100%" @selection-change="xuan">
          ...
          <el-table-column prop="t_id" label="编号" align="center" width="150" />
          <el-table-column prop="b_name" label="品牌名称" align="center" width="240" />
...
<script>
  ...
  export default {
    ...
    methods: {
      http(param) {
        $http("/market/brand", param, "POST").then((data) => {
      ...
      del() {
        this.dialog = false;
        //发起ajax请求删除数据
        $http("/market/brand_delete", { id: this.del_id }).then(data => {
          if (data.code != 400) {
